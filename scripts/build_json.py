...
import json, datetime, hashlib, re, os
from pathlib import Path

INPUT  = Path("data/bgg_data.json")
OUTPUT = Path("data/games_full.json")

MIN_BUILD = int(os.getenv("BUILD_MIN_ITEMS", "10"))  # 新增：最低輸出門檻
...
def make_id(r: dict) -> str:
    ...
    if ovr:
        ...
        return f"{base}-{_h.md5(str(ovr).encode('utf-8')).hexdigest()[:8]}"
...
def main():
    ...
    # 新增：若來源太少，保留舊檔，避免前端 0 筆
    if len(rows) < MIN_BUILD and OUTPUT.exists():
        old = json.loads(OUTPUT.read_text(encoding="utf-8"))
        print(f"BUILD GUARD: input {len(rows)} (<{MIN_BUILD}); keep previous games_full.json ({len(old)})")
        return
